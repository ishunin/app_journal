 ...
 <table id="#example1_one_disk" class="table table-bordered table-striped">
     <thead>
         <tr>
             <!-- Заголовок таблицы -->
     ...
     <?php
        $id_disk = $_GET['id'];
        $query = "SELECT *  FROM `disk_balance` WHERE `id_disk`=$id_disk AND `deleted`=0 ORDER BY `date` DESC";
        if (isset($_GET['state']) && !empty($_GET['state'])) {
            $state =$_GET['state'];
            $query = "SELECT *  FROM `disk_balance` WHERE `id_disk`=$id_disk AND `deleted`=0 AND `state`= $state ORDER BY `date` DESC";
        }

        $sql = mysqli_query($link_account, $query);
        echo '<tbody>';
        while ($result = mysqli_fetch_array($sql)) {
            ...
            echo '<tr class="' . $item_class . '">
                ...
                "<td>$id</td>" .
                "<td><a class='dropdown-item' href='one_disk_exact.php?id=" . $id . "&&id_disk=" . $id_disk . "' onclick=''>$serial_num</a></td>" .
                "<td>$state</td>" .
                "<td>$status</td>" .
                "<td>$point</td>" .
                "<td>$INM</td>" .
