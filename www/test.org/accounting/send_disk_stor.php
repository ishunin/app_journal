...
(isset($_POST['serial_num_new_disk']) && !empty($_POST['serial_num_new_disk'])) ? $serial_num_new_disk = check_input($link_account,$_POST['serial_num_new_disk']) : $serial_num_new_disk = '';
if ($_POST['state']==3) {
    $type_oper = 11;
}
else {
...
VALUES ($id,$id_templ,$type_oper,'',$state,1,2,'','',$id_user,0,NOW(),'$comment',0)";
$query = mysqli_query($link_account,$sql);
    if ($query) {
        #Обновляем
        $sql2 = "
        UPDATE `disk_balance` SET `status`=1, `state`=$state, `serial_num_new_disk`='$serial_num_new_disk', `point`=2 WHERE `id`=$id";
