 ...
 <table id="example1_b" class="table table-bordered table-striped">
     <thead>
         <tr>
             <!-- Заголовок таблицы -->
     ...
     <?php
        $sql = mysqli_query($link_account, "SELECT *  FROM `disk_balance` WHERE `status`=1 AND (`state`=3 ) AND `deleted`=0 ORDER BY `date` DESC LIMIT 100");
        echo '<tbody>';
