...
VALUES ($id,$id_templ,14,'',3,4,3,'','',$id_user,0,NOW(),'$comment',0)";
    $query = mysqli_query($link_account, $sql);
    if ($query) {
        #Обновляем
        $sql2 = "
        UPDATE `disk_balance` SET `status`=4,  `point`=3 WHERE `id`=$id";
        $query2 = mysqli_query($link_account, $sql2);
        if ($query2) {
            $_SESSION['success_action'] = 2;
            if ($page_back == 'one_disk_exact.php') {
                header('location:' . $page_back . '?id=' . $id . '&id_disk=' . $id_templ);
                exit();
            }
