 ...
 <table id="example1_k" class="table table-bordered table-striped">
     <thead>
         <tr>
             <!-- Заголовок таблицы -->
     ...
     <?php
        $sql = mysqli_query($link_account, "SELECT *  FROM `disk_balance` WHERE `status`=1 AND (`state`=4) AND `deleted`=0 ORDER BY `date` DESC LIMIT 100");
        echo '<tbody>';
        while ($result = mysqli_fetch_array($sql)) {
            ...
            #кнопки управления        
            $page_back = 'warehouse_broken_disks.php';
