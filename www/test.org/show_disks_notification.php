...
$str_note = '';
$count_disks = 0;
$sql = mysqli_query($link_account, "SELECT * FROM `disk_movement` WHERE `state`=1 AND `status`=1 AND `deleted`=0 AND `type_oper`=1 AND  `date`>= DATE_SUB(CURRENT_DATE, INTERVAL 1 DAY)  ORDER BY `date` DESC LIMIT 4");
