...
if (isset($_GET['ID']) && (!empty($_GET['ID']))) {
  $sql = mysqli_query($link, 'SELECT `ID`, `id_rec`, `id_shift`,`jira_num`, `content`,`action`, `id_user`,
    `destination`, `status`,`importance`,`keep`,`create_date`,`edit_date`  FROM `list` WHERE `id_shift`=' . $_GET['ID'] . '');
  ...
  if ($result) {
    ...
    $uploads = show_upload_files($link, $id, 2, 'one_page_jobs.php?ID=' . $id);
    echo '
  <div class="card">
...
echo '         
 <div class="card-footer">
  ...
  <div class="input-group input-group-sm mb-0">
        ...
        <input maxlength="1000"class="form-control form-control-sm" placeholder="Введите комментарий..." name="content" required>
...
<script type="text/javascript">
  function showMessage(id) {
    ...
    $('#modal_edit').load('ajax.php?red_id=' + id + '&page_back=one_page_incident.php', function() {
    ...
    <div class="modal-dialog">
      <div class="modal-content bg-danger">
        ...
        <div class="modal-footer justify-content-between">
          ...
          <input type="hidden" name="page_back" value="index.php">
        </div>
      </div>
      <!-- /.modal-content -->
    </div>
