 ...
 <table id="example1" class="table table-bordered table-striped col-md-9" style="table-layout: fixed;">
     <thead>
         <tr>
             <!-- Заголовок таблицы -->
     ...
     <?php
        $sql = mysqli_query($link_account, "SELECT *  FROM `disk_movement` WHERE `id_user` $query_option ORDER BY `date` DESC LIMIT 100");
        echo '<tbody>';
        while ($result = mysqli_fetch_array($sql)) {
            ...
            $sql2 = mysqli_query($link_account, "SELECT *  FROM `disk_balance` WHERE `id`=$id_disk");
