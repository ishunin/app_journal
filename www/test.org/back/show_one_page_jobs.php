...
if (isset($_GET['ID']) && (!empty($_GET['ID']))) {
  $_GET['ID']=intval($_GET['ID']);
    ...
    `keep`, `Create_date`,`importance`,`keep`,`create_date`,`edit_date`, `type`,`executor`,`start_task`,`end_task`  FROM `jobs` WHERE `ID`='.$_GET['ID']);
...
if ($result['executor']==0) {
    $executor = "Не имеет значения";
      ...
      $sql_user_executor = mysqli_query($link, "SELECT * FROM users WHERE `ID`=".$result['executor']."");
      if ($sql_user) {
...
<div class="active tab-pane" id="activity">
  ...
  <div class="post" style="border-bottom:none;">
    ...
    <blockquote '.$str_class.'">
      ...
      <p> Тип: '.get_type_jobs($result['type']).'</p>
 </blockquote>.
 </small>
    <h3>'.$result['theme'].'</h3>
    <p>
 ...
 <p>
      ...
      <a href="#" class="link-black text-sm mr-2" onclick="showMessage('.$result['ID'].');"><i class="far fa-edit"></i>Редактировать</a>
       <a href="one_page_jobs_print.php?ID='.$result['ID'].'" target="_blank" class="link-black text-sm mr-2"><i class="fas fa-print"></i> Печать</a>
