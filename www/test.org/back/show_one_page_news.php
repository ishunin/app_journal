...
if (isset($_GET['ID']) && (!empty($_GET['ID']))) {
  ...
  $sql = mysqli_query($link, 'SELECT `ID`, `theme`, `description`,`content`, `id_user`,`status`, `importance`,
    `keep`, `Create_date`,`importance`,`keep`,`create_date`,`edit_date`, `type`  FROM `news` WHERE `ID`=' . $_GET['ID']);
...
<div class="active tab-pane" id="activity">
 ...
 <a href="one_page_news_print.php?ID=' . $result['ID'] . '" target="_blank" class="link-black text-sm mr-2"><i class="fas fa-print"></i> Печать</a>
