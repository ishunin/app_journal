...
if (isset($_POST['Theme']) && !empty($_POST['Theme']) && isset($_POST['Area_content']) && !empty($_POST['Area_content'])) {
  ...
  #(1, 1, '{$_POST['Jira_num']}', '{$_POST['Area_content']}', '{$_POST['Action']}',1,'{$_POST['Destination']}', {$_POST['Status']}, {$_POST['Importance']},{$_POST['Keep']}, NOW(), NOW())");
    ...
    $sql = mysqli_query($link, "UPDATE `news` SET `theme` = '{$_POST['Theme']}', `description` = '{$_POST['Area_description']}',
    `content`='{$_POST['Area_content']}',`id_user`='{$_COOKIE['id']}', `status` = '{$_POST['Status']}', `importance` = '{$_POST['Importance']}',`keep`='{$_POST['Keep']}',`edit_date`= now(),`type`='{$_POST['Type']}' WHERE `ID`='{$_POST['id']}'");
