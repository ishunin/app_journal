...
$sql = mysqli_query($link, "SELECT `ID`, `theme`, `description`,`content`, `id_user`,`status`, `importance`,
    `keep`, `Create_date`,`importance`,`keep`,`create_date`,`edit_date`, `type`,`deleted` 
    FROM `news` WHERE id_user $query_option AND `deleted`=0 order by `create_date` DESC LIMIT 50");
while ($result = mysqli_fetch_array($sql)) {
  ...
  echo '
      ...
      <small><a href="one_page_news.php?ID=' . $result['ID'] . '"><i class="fas fa-share mr-1"></i> Перейти</a></small>
    </div>
    <!-- /.post -->                             
      ';
}
